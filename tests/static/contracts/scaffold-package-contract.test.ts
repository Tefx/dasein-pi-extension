...
test("SettingsList/getSettingsListTheme imports resolve only through the approved Pi peer dependency", async () => {
  ...
  for (const sourceFile of listSourceFiles("src")) {
    ...
    const settingsImportLines = sourceText
      .split("\n")
      .filter((line) => /import\s+.*(?:SettingsList|getSettingsListTheme).*from/.test(line));
  ...
  const resolvedPeer = await import.meta.resolve("@earendil-works/pi-tui");
  assert.match(resolvedPeer, /@earendil-works\/pi-tui/);
});
