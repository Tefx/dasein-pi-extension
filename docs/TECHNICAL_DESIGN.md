...
Evidence status is a non-empty set/list, not a single scalar, because a mechanism may be API-verified while still pending live smoke. Status payloads must preserve that distinction instead of collapsing it.

Current verification evidence:

| Mechanism | Pi version | Binary path | Evidence statuses | Verification date | Observed behavior / source evidence | Release requirement |
|---|---:|---|---|---|---|---|
| `pi.registerCommand` for slash commands with raw args and completions | `0.78.1` | `/opt/homebrew/bin/pi` | `SOURCE_VERIFIED`, `LIVE_SMOKE_PENDING` | 2026-06-06 | `docs/extensions.md` documents `pi.registerCommand("name", ...)`; examples including `examples/extensions/commands.ts` and `qna.ts` register slash commands. | Live command registration smoke before release. |
| `pi.registerFlag` with `type: "string"` for launch arguments | `0.78.1` | `/opt/homebrew/bin/pi` | `SOURCE_VERIFIED`, `LIVE_SMOKE_PENDING` | 2026-06-06 | `docs/extensions.md` lists `pi.registerFlag("my-flag", ...)`; `examples/extensions/ssh.ts` registers a string flag and resolves it on `session_start`. | Live launch-flag smoke before release. |
| `pi.on("context")` before each LLM call for non-destructive message injection | `0.78.1` | `/opt/homebrew/bin/pi` | `SOURCE_VERIFIED`, `LIVE_SMOKE_PENDING` | 2026-06-06 | `docs/extensions.md` lifecycle shows `context` before provider request and describes mutable copied messages. | Live context-hook smoke before release. |
| `CustomMessage display:false` conversion to LLM user message through Pi `convertToLlm` | `0.78.1` | `/opt/homebrew/bin/pi` | `SOURCE_VERIFIED`, `LIVE_SMOKE_PENDING` | 2026-06-06 | `docs/extensions.md` documents custom message injection shape; local Pi source/docs observation indicates hidden custom messages are conversion inputs rather than UI-visible chat entries. | Live hidden-message conversion smoke before release. |
| `pi.events` for cross-extension state publishing | `0.78.1` | `/opt/homebrew/bin/pi` | `SOURCE_VERIFIED`, `LIVE_SMOKE_PENDING` | 2026-06-06 | `examples/extensions/event-bus.ts` emits and receives extension events. | Live event-bus smoke before release. |
| `ctx.ui.setStatus` for persistent footer status | `0.78.1` | `/opt/homebrew/bin/pi` | `SOURCE_VERIFIED`, `LIVE_SMOKE_PENDING` | 2026-06-06 | `docs/extensions.md` documents `ctx.ui.setStatus`; examples include `examples/extensions/status-line.ts` and `rpc-demo.ts`. | Live TUI status smoke before release. |
| `ctx.ui.setWidget` for optional editor-adjacent context display | `0.78.1` | `/opt/homebrew/bin/pi` | `SOURCE_VERIFIED`, `LIVE_SMOKE_PENDING` | 2026-06-06 | `docs/extensions.md` documents `ctx.ui.setWidget`; examples include `examples/extensions/widget-placement.ts` and `rpc-demo.ts`. | Live TUI widget smoke before release. |
| `ctx.ui.custom` plus `SettingsList` for interactive configuration UI | `0.78.1` | `/opt/homebrew/bin/pi` | `API_VERIFIED`, `LIVE_SMOKE_PENDING` | 2026-06-06 | `docs/extensions.md` documents `ctx.ui.custom`; examples use custom UI. `SettingsList` API/source shape is verified, but rendering and interaction are not live-verified. | Blocking live SettingsList smoke before release. |
| `ctx.mode === "tui"` guard for TUI-only rendering | `0.78.1` | `/opt/homebrew/bin/pi` | `SOURCE_VERIFIED`, `LIVE_SMOKE_PENDING` | 2026-06-06 | `docs/extensions.md` states current mode can be `"tui"`, `"rpc"`, `"json"`, or `"print"`; `examples/extensions/custom-header.ts` guards with `ctx.mode === "tui"`. | Live non-TUI no-op smoke before release. |
| `session_start` and `session_shutdown` for lifecycle and cleanup | `0.78.1` | `/opt/homebrew/bin/pi` | `SOURCE_VERIFIED`, `LIVE_SMOKE_PENDING` | 2026-06-06 | `docs/extensions.md` lifecycle documents both events; examples include `auto-commit-on-exit.ts`, `mac-system-theme.ts`, and `todo.ts`. | Live lifecycle smoke before release. |
| Human input observation through `input` when available, otherwise `before_agent_start` as fallback sampling point | `0.78.1` | `/opt/homebrew/bin/pi` | `SOURCE_VERIFIED`, `LIVE_SMOKE_PENDING` | 2026-06-06 | `docs/extensions.md` documents `input` processing before skill/template expansion and `before_agent_start` before agent processing. | Live lifecycle observation smoke before release. |
| Agent completion observation through `agent_end` | `0.78.1` | `/opt/homebrew/bin/pi` | `SOURCE_VERIFIED`, `LIVE_SMOKE_PENDING` | 2026-06-06 | `docs/extensions.md` lifecycle documents `agent_end`; examples include `git-checkpoint.ts` and `border-status-editor.ts`. | Live agent-end smoke before release. |
...
Support claims must not ship on `SOURCE_VERIFIED` or `API_VERIFIED` evidence alone. Initial release requires live smoke evidence for command registration, flag parsing, context injection, event publishing, TUI status/widget/custom UI behavior, and lifecycle observation. If a Pi installation does not expose one of these mechanisms, Dasein must fail the affected feature closed and report it in `/dasein status`. This table is current evidence, not a permanent compatibility guarantee.

## Design Decisions

### Decision 1: `context` hook for injection

Dasein injects ambient context using Pi's `context` hook because it fires before each LLM call and operates on Pi-level messages.

Rationale: this avoids provider-specific payload mutation and keeps the design portable across providers.

Trade-off: ambient context is converted to a user message, not a provider-native system field.

### Decision 2: Neutral injected label

Dasein's default injected label is:

```text
ambient_ctx
```

The default injected message is:
...
1. Pi fires `context` before each LLM call.
2. Dasein injector reads only the pre-rendered in-memory `stateStore.getRenderedContext().agent` string (or the equivalent `getRenderedAgentString()` convenience method).
3. If the string is empty or null, it returns no changes. When `core.agentInjectionEnabled === false`, the renderer/state store has already exposed the string as empty/null.
4. If non-empty, it appends a hidden custom message to the copied message list.
5. Pi converts the custom message to a user message through `convertToLlm`.
...
- `src/core/injector.ts` uses an explicit import allowlist: core ambient-message types, the `DaseinStateStore` interface, and renderer output types only.
...
- The injector must not trigger sensor refresh, sensor action, sensor cleanup, sensor discovery, config read, config write, durable state read, durable state write, or native/helper import. It reads only the pre-rendered in-memory string/types and appends a message or returns no change.
...
In the current Pi Node installation, Pi's extension loader uses `jiti` and dynamic `.ts` imports work from an already-loaded extension. This is `SOURCE_VERIFIED`/`API_VERIFIED` evidence until live reload smoke confirms it. Initial release requires a live Pi process smoke gate proving directory/package install discovery of user-added `.ts` sensors and `/dasein reload` cache-busting of changed sensor modules.

Changed sensor files may require cache-busting import:

```text
<absolute-sensor-path>?reload=<timestamp>
```
...
The macOS helper source is CoreLocation for coordinates/accuracy/permission/timestamp and best-effort `CLGeocoder` reverse geocoding for placemark fields. Reverse geocoding is optional helper output: lack of placemark data must not turn a valid coordinate fix into an error.
...
```json
{
  ...
  "scripts": {
    ...
    "test": "node --import tsx --test tests/unit/**/*.test.ts tests/integration/**/*.test.ts tests/behavior/**/*.test.ts tests/static/**/*.test.ts",
    "test:file": "node --import tsx --test",
    "test:native": "node --import tsx --test tests/native/**/*.test.ts",
    "test:smoke": "node --import tsx --test tests/smoke/**/*.test.ts"
  }
}
```

Downstream implementation may add build or lint scripts, but the five scripts above are required and must remain stable for gates. Ordinary `npm test` is the full all-platform CI gate and must not require a live Pi TUI; `npm run test:file -- <files>` is for file-scoped matrix rows; `npm run test:smoke` is the release smoke gate. The package/static gate must fail if runtime `dependencies` is present and non-empty; if Pi-owned runtime imports are missing from `peerDependencies` or use any range other than `"*"`; if a non-Pi runtime dependency is declared without a design update; if root `index.ts` is missing or does more than delegate to `./src/index.ts`; if `package.json.pi.extensions`, when present, does not include `./index.ts`; if the root extension entrypoint is not loadable from a symlinked `~/.pi/agent/extensions/dasein` install; or if that symlinked load cannot resolve the documented SettingsList/getSettingsListTheme imports. `devDependencies` are permitted for TypeScript/test tooling only.
...
| Pi integration smoke | `tests/integration/pi-extension.test.ts` | `npm run test:file -- tests/integration/pi-extension.test.ts` | Requires Pi `0.78.1` or later unless compatibility tests explicitly expand support; records mechanism evidence as non-empty `evidenceStatuses` lists containing `SOURCE_VERIFIED`, `API_VERIFIED`, `LIVE_SMOKE_PENDING`, and/or `LIVE_SMOKE_VERIFIED`; docs/source-only evidence never reports live verification; fake Pi API may prove registration shape but cannot satisfy live support claims; registers `/dasein`; registers `--dasein` string flag; context hook appends hidden custom message; custom message converts to LLM user message; TUI status/widget calls are guarded by `ctx.mode === "tui"`; `core.statusEnabled=false` calls `ctx.ui.setStatus("dasein", undefined)` and `core.widgetEnabled=false` calls `ctx.ui.setWidget("dasein", undefined)`; SettingsList remains API/source verified plus live-smoke pending until live smoke runs. | All platforms, with Pi APIs faked when Pi is unavailable |
| macOS native helper | `tests/native/macos-location-helper.test.ts` | `npm run test:native -- tests/native/macos-location-helper.test.ts` | Swift helper typechecks; directory/package installs use `<extension_root>/src/native/macos-location-helper.swift`; single-file packaged installs either expose a materialized packaged helper file path or geo fails closed with `SensorError.kind: "helper-unavailable"` and no helper spawn; timeout is `3000ms`; stdout/stderr cap at `16KiB`; stdout JSON returns `lat`, `lon`, `accuracy_m`, `permission`, `timestamp`, and optional `placemark`; timeout/process/permission errors increment backoff; success resets backoff; manual refresh bypasses delay but not timeout; no helper spawns during backoff unless manual; no orphan helper processes after timeout/shutdown. | Run only on `process.platform === "darwin"`; skip with explicit reason on non-macOS |
| Pi SettingsList live smoke release gate | `tests/smoke/pi-settings-list.test.ts` | `npm run test:smoke -- tests/smoke/pi-settings-list.test.ts` | Runs against a live Pi `0.78.1+` TUI environment; opens the Dasein settings UI; verifies SettingsList renders sensor inspectability metadata before enablement, including remote destinations, payload classes, transmission cadence, disable control, declared background work, and `effectiveIntervalMs`; verifies core toggles, common sensor fields, visible recurring interval controls, simple sensor fields, and valid `external.<key>.ui`/`external.<key>.agent` controls; verifies object/array/map-like fields such as `geo.tags` and `lapse.agentFields` are not exposed as flat SettingsList controls; toggles one setting; verifies the change enters the FIFO config mutation queue and persists the canonical path only. | Blocking release gate with live Pi TUI required; not satisfied by fake Pi API alone and not run by ordinary `npm test` |
| Pi dynamic sensor reload live smoke release gate | `tests/smoke/pi-dynamic-reload.test.ts` | `npm run test:smoke -- tests/smoke/pi-dynamic-reload.test.ts` | Runs against a real Pi process in directory/package install mode; adds or changes a local `.ts` sensor under `<extension_root>/src/sensors`; verifies startup/manual `/dasein reload` discovers it, cache-busting imports the changed module, load failure keeps the old registry/rendered context, and single-file packaged mode does not claim dynamic user-added sensor discovery. | Blocking release gate with live Pi process required; not satisfied by fake Pi API alone and not run by ordinary `npm test` |
