...
| `pi.on("context")` before each LLM call for non-destructive message injection | `0.78.1` | `/opt/homebrew/bin/pi` | `SOURCE_VERIFIED`, `LIVE_SMOKE_VERIFIED` | 2026-06-06 | `docs/extensions.md` documents the context hook. Live ledger row `pi.context.hidden-custom-message=PROVEN`; artifact `context-injection-proof.json` shows a hidden Dasein custom message appended in the live Pi context hook. | Re-run `npm run test:smoke` for release evidence. |
| `CustomMessage display:false` conversion to LLM user message through Pi `convertToLlm` | `0.78.1` | `/opt/homebrew/bin/pi` | `SOURCE_VERIFIED`, `LIVE_SMOKE_VERIFIED` | 2026-06-06 | Artifact `context-injection-proof.json` shows `hiddenDaseinCustomMessage=true` and Pi `convertToLlm` converted the hidden ambient message to a quiet LLM user context with no raw `[ambient_ctx:` prefix. | Re-run `npm run test:smoke` for release evidence. |
...
This string is the renderer's canonical ambient-context representation, not default human-facing chrome. Before request-path injection, Dasein wraps the content as quiet hidden context with instructions not to mention, quote, label, or summarize it unless the user explicitly asks about Dasein ambient context. In TUI mode, Dasein must not publish the raw `[ambient_ctx: ...]` string through the visible status footer or default settings surface. Human-facing status defaults to a short Dasein summary, while the raw renderer payload remains inspectable only through explicit diagnostics such as `/dasein status` proof data and debug artifacts.
...
| Pi live mechanism and SettingsList smoke release gate | `tests/smoke/contracts/live-pi-smoke-gate-contract.test.ts` | `npm run test:smoke` | Runs against a live Pi `0.78.1+` TUI/process environment; proves command registration, `--dasein`, context hidden-message conversion, `pi.events`, status/widget render and cleanup, lifecycle observation, non-TUI fallback, SettingsList common controls, inspectability metadata before enablement, canonical-path persistence, `ctx.ui.custom` without provider/API key, and no fake-host conflation. Generates `.dasein/live-pi-smoke/latest/checklist_receipt.json` with 11/11 proven rows and zero blockers when successful. | Blocking release gate with live Pi TUI/process required; not satisfied by fake Pi API alone and not run by ordinary `npm test` |
