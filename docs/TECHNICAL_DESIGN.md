...
Dasein injects agent ambient context through `core.agentInjectionTransport`. The safer default is `auto`, which observes Pi's selected model, resolves an effective transport from local model metadata plus the generated offline cache-capability table, and chooses `providerPayload` for cache-capable/cache-signalled models or `systemPrompt` for unknown/non-cache models. The explicit `providerPayload` mode always uses the provider-payload path. The explicit `systemPrompt` mode appends a bounded dynamic block to Pi's per-turn `before_agent_start` `systemPrompt`.
...
5. In explicit `providerPayload` mode, if non-empty, Dasein appends only a stable Dasein policy to `systemPrompt` during `before_agent_start`; the dynamic ambient block is injected later in `before_provider_request` only when the final payload shape is OpenAI Responses (`input[]`) or OpenAI-compatible Chat Completions (`messages[]`). In `auto` mode, Dasein first resolves the selected model: cache-capable/cache-signalled models follow the provider-payload path, while unknown/non-cache models follow the `systemPrompt` path.
