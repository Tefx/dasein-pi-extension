...
Trade-off: automatic hot-plug is not supported. Core code changes still require Pi's global extension reload/restart flow (not Dasein's `/dasein reload` sensor/config command), and packaged single-file users must switch to directory install for local sensors.
...
Default injected system prompt block example:
...
- `/dasein inspect agent` output must identify `target: "agent"`, `source: "pre-rendered-memory"`, whether agent injection is enabled, the configured injected label, the current rendered agent string, the exact `<DaseinAmbientContext>` system prompt block when present, and truncation/omitted-key metadata.
...
Directory/package installs use the source helper path under the extension root to build an ignored local `.app` bundle before geo collection. The app bundle contains `NSLocationWhenInUseUsageDescription`, initializes AppKit before requesting CoreLocation authorization, and waits for the authorization callback before requesting a one-shot location. This is required so macOS Location Services attributes permission to `Dasein Location Helper` instead of to `swift-frontend` or an anonymous CLI executable. Single-file packaged installs do not assume `<extension_root>/src/native/...` exists; they either expose a materialized packaged helper file path during startup or geo native collection fails closed with `SensorError.kind: "helper-unavailable"` and no helper spawn. This failure does not disable Dasein core, clock, lapse, external state, or non-geo UI.
