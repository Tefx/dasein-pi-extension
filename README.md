...
Agent injection mechanism: by default Dasein uses `core.agentInjectionTransport=auto`. At runtime `auto` observes Pi's selected model metadata, checks the offline generated cache-capability table, and chooses `providerPayload` for cache-capable/cache-signalled models; otherwise it falls back to the legacy `systemPrompt` path. You can switch `core.agentInjectionTransport` to `providerPayload` for explicit provider-payload-only behavior, `systemPrompt` for explicit system-prompt behavior, or `off` to disable agent injection. Dasein does not use Pi `CustomMessage` entries for ambient context.
